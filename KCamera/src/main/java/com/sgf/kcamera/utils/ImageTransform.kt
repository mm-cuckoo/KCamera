...
class ImageTransform {
    ...
    private var yRawSrcBytes = ByteArray(0)
    private var uRawSrcBytes = ByteArray(0)
    private var vRawSrcBytes = ByteArray(0)
    private var ySrcBytes = ByteArray(0)
    private var vSrcBytes = ByteArray(0)
    ...
    fun yuv420888ToNV21(image: Image, putNv21: ByteArray) : ByteArray {
        ...
        //分别准备三个数组接收YUV分量。
        if (yRawSrcBytes.size != remaining0) {
            yRawSrcBytes = ByteArray(remaining0)
        }

        if (uRawSrcBytes.size != remaining1) {
            uRawSrcBytes = ByteArray(remaining1)
        }

        if (vRawSrcBytes.size != remaining2) {
        ...
        if (pixelStride == w) {
            ...
            val ySize = w * h
            if (ySrcBytes.size != ySize) {
                ySrcBytes = ByteArray(w * h)
            }
            val vSize = w * h / 2 - 1
            if (vSrcBytes.size != vSize) {
